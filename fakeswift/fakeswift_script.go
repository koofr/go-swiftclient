...
swift.connect().then(function() {
  return swift.addAccount('test');
}).then(function() {
  return swift.addUser('test', 'tester', 'testing');
}).then(function() {
  return swift.addContainer('test', 'test-container');
}).then(function() {
  return swift.importObjects('test', 'test-container', [{
    name: 'file.txt',
